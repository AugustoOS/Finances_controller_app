...
```json
{
  ...
  "faturas":      { "ct1|2026-08": true,
                    "ct1|2026-09": [ { "valor": 400, "em": "2026-09-09T14:32" } ] },
...
| `faturas` | One key per bill that has been paid, `cardId|month`, holding the list of payments made against it: `valor` and `em`, the stamp of when it was registered. Older files may carry `true`, meaning settled in one go, or a bare number, and both still work. A bill missing from here has had nothing paid, and whatever is still owed on any bill keeps holding limit. |
| `emprestimos` | Loans paid in fixed instalments. `valorParcela` is the monthly amount, `parcelas` how many there are, `inicio` the month of the first one and `dia` the day it falls due; `pagas` holds one key per month already settled, whose value is the stamp of when it was marked. |
